...
echo "Creating todomailer secrets..."
vault kv put secret/todomailer \
  DB_USER=todomailer \
  DB_PASS=todomailerpass

if [ $? -eq 0 ]; then
  echo "✓ Created secret/todomailer"
else
  echo "✗ Failed to create secret/todomailer"
  exit 1
fi
