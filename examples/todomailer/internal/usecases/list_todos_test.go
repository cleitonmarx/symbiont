...
func TestListTodosImpl_Query(t *testing.T) {
	tests := map[string]struct {
		setExpectations func(repo *domain.MockRepository)
		page            int
		pageSize        int
...
func TestInitListTodos_Initialize(t *testing.T) {
	...
	expectedListTodos := NewListTodosImpl(repo)
	...
	assert.NoError(t, err)
	assert.EqualExportedValues(t, expectedListTodos, registeredListTodos)
}
